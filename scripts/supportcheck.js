...
(function () {
    ...
    // 检测是否为 UIWebView
    var isUIWebView = /iPhone|iPad|iPod/i.test(navigator.userAgent) && 
                      !(window.webkit && window.webkit.messageHandlers) &&
                      !window.chrome;

    // UIWebView 兼容处理：跳过部分检测
    if (isUIWebView) {
        console.warn("UIWebView detected: skipping advanced feature checks.");
    ...
    if (missingFeatures.length === 0 && !isKasperskyScriptInjected) {
        ...
        if (/android/i.test(ua)) {
            ...
            msgText += "<br><br>Note: using the <strong>iOS simulator</strong> requires <strong>Xcode 12+</strong>. Otherwise try testing on a real device instead.";
                   ...
                   /trident/i.test(ua) || /edge\//i.test(ua)) {
